...
bool OS_Opacity_Manager::opacifyByControl_FullObservation(MDGraph& sog, stack<Pair> st, bdd src, bdd dest, int t, Class_Of_State* a_, bdd S, string trace) {
/*
****************** Optimal version **********************
*/
Class_Of_State* a;//pred de a_
	...
	if (a_->class_state == test) {
        ...
        vector<OS_Supervisor> supervisors = manager.getManagedSupervisors();//getManagedSupervisors; in our case we have an only one supervisor
        OS_Supervisor supervisor = supervisors[0];//supervisor
        set <int> control = supervisor.getControllable(); //set of controllable events //This is not the supervision fct be careful! Change name preferably

        if (control.find(t) != control.end()) { //t is controllable by the supervisor
            cout<<"t"<<t<<" is controllable "<<endl;
            manager.desactivate_FullObs(&supervisor, t, trace, src);
        ...
        else if (st.size() > 1)
        {
           ...
           //// cout<<"st.top().first.first "<<st.top().first.first->class_state<<" succ size of st.top : "<< st.top().first.first->Successors.size() <<endl;
            ...
            //sog.currentstate->class_state = bddtrue;
            if (opacifyByControl_FullObservation(sog, st, src, dest, t, a_, S, trace)) {
...
*
*/
void OS_Opacity_Manager::computeAndOpacify_SOG_FullObservation(MDGraph& g) {
    ...
    */
	...
	do {
        ...
        if (e.second.size()==1){cout<<"no enabled event is possible"<<endl;st.pop();}
			...
			//there is an observable event reached by c
        {
			...
			if (pos) {
				...
				const bool test = this->attackers.front().observable.find(t) != this->attackers.front().observable.end();
				if (T == Complete_meta_state && test) {
					//a included in secret bdd
					//opacity is leaked
...
//					cout<<"bdd of the reached class before src "<<reached_class->class_state<<endl;
					...
					bdd dest = this->model->get_successor(src, t) ;
					...
					if (opacifyByControl(g, st_, src, dest, t, reached_class, this->model->SecretBdd, tr))
                        cout<<"Opacify returns True nothing to be done"<<endl;
						...
						tr = tr + "-->" + "t" + a;
						cout << '\n' << "The system's opacity cannot be enforced by control [after the trace  :  " << tr << "]" << endl;
						//-------------end not needed--------------//
					...
					char a = (char)(((int)'0')+t);
					tr =tr +"-->"+"t"+a;
					g.addArc(); cout << "added an arc here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
