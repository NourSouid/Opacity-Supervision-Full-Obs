...
int MDGraph::getTrans(Class_Of_State* b, Class_Of_State* a){
  //  cout<<"get transition$$$$$$$$$$$$$$$$$$$$$$"<<endl;
  //  cout<<"in get Transition fct: pred of a : "<<a->Predecessors.size()<<"its pred : "<<a->Predecessors.front().first->class_state<<endl;
  //  cout<<"st.top : "<<b->class_state<<endl;



    for(std::vector< std::pair<Class_Of_State*,int> >::iterator it = a->Predecessors.begin(); it != a->Predecessors.end(); ++it) {
        //std::cout << "pred"<<endl;
        //cout<< (*it).first->class_state<<endl;
